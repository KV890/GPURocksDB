...
__host__ __device__ inline uint32_t GPUExtend(uint32_t init_crc,
                                              const char* data, size_t n);

__host__ __device__ inline uint32_t GPUValue(const char* data, size_t n) {
  return GPUExtend(0, data, n);
}

static const uint32_t kGPUMaskDelta = 0xa282ead8ul;

__host__ __device__ inline uint32_t GPUMask(uint32_t crc) {
  return ((crc >> 15) | (crc << 17)) + kGPUMaskDelta;
}

__host__ __device__ inline uint32_t GPUUnmask(uint32_t masked_crc) {
  uint32_t rot = masked_crc - kGPUMaskDelta;
  return ((rot >> 17) | (rot << 15));
}
__device__ static const uint32_t gpu_table0_[256] = {
...
__device__ static const uint32_t gpu_table1_[256] = {
...
__device__ static const uint32_t gpu_table2_[256] = {
...
__device__ static const uint32_t gpu_table3_[256] = {
...
__host__ __device__ static inline uint32_t GPU_LE_LOAD32(const uint8_t* p) {
  return GPUDecodeFixed32(reinterpret_cast<const char*>(p));
}

__host__ __device__ inline uint32_t GPUExtend(uint32_t crc, const char* buf,
                                              size_t size) {
...
#define STEP4                                                  \
  do {                                                         \
    uint32_t c = l ^ GPU_LE_LOAD32(p);                         \
    p += 4;                                                    \
    l = gpu_table3_[c & 0xff] ^ gpu_table2_[(c >> 8) & 0xff] ^ \
        gpu_table1_[(c >> 16) & 0xff] ^ gpu_table0_[c >> 24];  \
  } while (0)
