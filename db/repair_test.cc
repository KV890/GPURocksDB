...
TEST_F(RepairTest, LostManifest) {
  ...
  ASSERT_OK(Flush());
  // Need to get path before Close() deletes db_with_cf, but delete it after Close() to
...
TEST_F(RepairTest, CorruptManifest) {
  ...
  ASSERT_OK(Put("key2", "val2"));
  ASSERT_OK(Flush());
  // Need to get path before Close() deletes db_with_cf, but overwrite it after Close()
