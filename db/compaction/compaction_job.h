...
#include "db/cuda/gpu_table_properties.cuh"
...
class CompactionJob {
 public:
  ...
  CompactionJob(
      ...
      bool paranoid_file_checks, bool measure_io_stats,
      const std::string& dbname, CompactionJobStats* compaction_job_stats,
      std::vector<SSTableInfo> _infos, std::vector<FileMetaData> _metas,
      std::vector<TableProperties> _tps,
  ...
  void AddInputFiles(size_t i, InputFile* input_files, FileMetaData* meta);
  ...
  Status MyFinishCompactionOutputFile(
      const Compaction* compact,
 ...
 protected:
  ...
  std::vector<TableProperties> tps;

  std::vector<std::shared_ptr<WritableFileWriter>> file_writers;
