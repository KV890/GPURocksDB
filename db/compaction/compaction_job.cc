...
#include "db/cuda/gpu_compaction.cuh"
#include "db/cuda/gpu_format.cuh"
...
CompactionJob::CompactionJob(
    ...
    std::shared_ptr<Cache> table_cache, EventLogger* event_logger,
    bool paranoid_file_checks, bool measure_io_stats, const std::string& dbname,
    CompactionJobStats* compaction_job_stats, std::vector<SSTableInfo> _infos,
    std::vector<FileMetaData> _metas, std::vector<TableProperties> _tps,
    std::vector<std::shared_ptr<WritableFileWriter>> _file_writers,
    std::vector<std::shared_ptr<TableBuilderOptions>> _tbs,
    Env::Priority thread_pri, const std::shared_ptr<IOTracer>& io_tracer,
    const std::atomic<bool>& manual_compaction_canceled,
    const std::string& db_id, std::string db_session_id,
    std::string full_history_ts_low, std::string trim_ts,
    BlobFileCompletionCallback* blob_callback, int* bg_compaction_scheduled,
    int* bg_bottom_compaction_scheduled)
    : compact_(new CompactionState(compaction)),
      ...
      metas(std::move(_metas)),
      tps(std::move(_tps)),
      file_writers(std::move(_file_writers)),
      tbs(std::move(_tbs)),
      is_gpu_compact(false),
      job_id_(job_id),
      dbname_(dbname),
      db_id_(db_id),
...
Status CompactionJob::FinishCompactionOutputFile(
  ...
  if (s.ok()) {
    ...
    meta->num_data_blocks = tp.num_data_blocks;
  }
...
Status CompactionJob::InstallCompactionResults(
  ...
  if (is_gpu_compact) {
    for (const auto& meta : metas) {
      edit->AddFile(compaction->output_level(), meta);
    }
  }
...
void CompactionJob::AddInputFiles(size_t i, InputFile* input_files,
                                  FileMetaData* meta) {
  std::string file_tmp;

  uint64_t file_number = meta->fd.GetNumber();

  std::string file_name = GetTableFileName(file_number);
  ReadFileToString(fs_.get(), file_name, &file_tmp);

  char* file_char = new char[file_tmp.size()];
  memcpy(file_char, file_tmp.c_str(), file_tmp.size());

  input_files->level = compact_->compaction->level(i);
  input_files->file = file_char;
  input_files->file_size = file_tmp.size();
  input_files->file_number = file_number;
  input_files->num_data_blocks = meta->num_data_blocks;
  input_files->num_entries = meta->num_entries;
}
...
void CompactionJob::OpenOutputFile(size_t i, const Compaction* compact,
  ...
  meta.smallest.DecodeFrom(Slice(input[0].key, keySize_ + 8));
  meta.largest.DecodeFrom(
      Slice(input[infos[i].total_num_kv - 1].key, keySize_ + 8));

  auto min = std::min_element(input.begin(), input.end(),
                              [](const GPUKeyValue& a, const GPUKeyValue& b) {
...
void CompactionJob::OpenOutputFile(size_t i, const Compaction* compact,
  ...
  metas[i].largest.DecodeFrom(Slice(largest_key, keySize_ + 8));
  metas[i].smallest.DecodeFrom(Slice(smallest_key, keySize_ + 8));

  metas[i].fd.largest_seqno = largest_seqno;
  metas[i].fd.smallest_seqno = smallest_seqno;
}

Status CompactionJob::MyOpenCompactionOutputFile(
    const Compaction* compact, std::shared_ptr<WritableFileWriter>& file_writer,
    FileMetaData& metaData, std::shared_ptr<TableBuilderOptions>& tboptions) {
  // no need to lock because VersionSet::next_file_number_ is atomic
  uint64_t file_number = versions_->NewFileNumber();
  std::string fname = GetTableFileName(file_number);
  // Fire events.
  ColumnFamilyData* cfd = compact->column_family_data();
  EventHelpers::NotifyTableFileCreationStarted(
      cfd->ioptions()->listeners, dbname_, cfd->GetName(), fname, job_id_,
      TableFileCreationReason::kCompaction);
  // Make the output file
  ...
  // Pass temperature of the last level files to FileSystem.
  FileOptions fo_copy = file_options_;
  Temperature temperature = compact->output_temperature();
  // only set for the last level compaction and also it's not output to
  // penultimate level (when preclude_last_level feature is enabled)
  if (temperature == Temperature::kUnknown && compact->is_last_level()) {
    temperature = compact->mutable_cf_options()->last_level_temperature;
  }
  fo_copy.temperature = temperature;
  ...
  if (!s.ok()) {
    ROCKS_LOG_ERROR(
        ...
        "[%s] [JOB %d] OpenCompactionOutputFiles for table #%" PRIu64
        " fails at NewWritableFile with status %s",
        compact->column_family_data()->GetName().c_str(), job_id_, file_number,
        s.ToString().c_str());
    LogFlush(db_options_.info_log);
    EventHelpers::LogAndNotifyTableFileCreationFinished(
        event_logger_, cfd->ioptions()->listeners, dbname_, cfd->GetName(),
  ...
  uint64_t oldest_ancester_time =
      compact->MinInputFileOldestAncesterTime(nullptr, nullptr);
  if (oldest_ancester_time == std::numeric_limits<uint64_t>::max()) {
    oldest_ancester_time = current_time;
  }

  // Initialize a SubcompactionState::Output and add it to sub_compact->outputs
  uint64_t epoch_number = compact->MinInputFileEpochNumber();
  {
    metaData.fd = FileDescriptor(file_number, compact->output_path_id(), 0);
    metaData.oldest_ancester_time = oldest_ancester_time;
    metaData.file_creation_time = current_time;
    metaData.epoch_number = epoch_number;
    metaData.temperature = temperature;
    assert(!db_id_.empty());
    assert(!db_session_id_.empty());
    s = GetSstInternalUniqueId(db_id_, db_session_id_, metaData.fd.GetNumber(),
                               &metaData.unique_id);
    if (!s.ok()) {
      ROCKS_LOG_ERROR(db_options_.info_log,
                      "[%s] [JOB %d] file #%" PRIu64
                      " failed to generate unique id: %s.",
                      cfd->GetName().c_str(), job_id_, metaData.fd.GetNumber(),
                      s.ToString().c_str());
      return s;
    }
  }
  ...
  writable_file->SetPreallocationBlockSize(
      static_cast<size_t>(compact->OutputFilePreallocationSize()));
  const auto& listeners = compact->immutable_options()->listeners;

  file_writer = std::make_unique<WritableFileWriter>(
      std::move(writable_file), fname, fo_copy, db_options_.clock, io_tracer_,
      db_options_.stats, listeners, db_options_.file_checksum_gen_factory.get(),
      tmp_set.Contains(FileType::kTableFile), false);

  tboptions = std::make_unique<TableBuilderOptions>(
      *cfd->ioptions(), *(compact->mutable_cf_options()),
      cfd->internal_comparator(), cfd->int_tbl_prop_collector_factories(),
      compact->output_compression(), compact->output_compression_opts(),
      cfd->GetID(), cfd->GetName(), compact->output_level(), bottommost_level_,
      TableFileCreationReason::kCompaction, 0 /* oldest_key_time */,
      current_time, db_id_, db_session_id_, compact->max_output_file_size(),
      file_number);
...
Status CompactionJob::MyFinishCompactionOutputFile(
    const Compaction* compact, std::shared_ptr<WritableFileWriter> file_writer,
    FileMetaData* meta, SSTableInfo* info, TableProperties* tp) {
  uint64_t output_number = meta->fd.GetNumber();
  assert(output_number != 0);
  ...
  const uint64_t current_entries = info->total_num_kv;

  meta->fd.file_size = info->file_size;
  ...
  if (meta->smallest.size() > 0 && meta->largest.size() > 0) {
    ...
    Slice new_largest = meta->largest.user_key();
    if (!new_largest.empty() && !new_smallest.empty()) {
      refined_oldest_ancester_time = compact->MinInputFileOldestAncesterTime(
          &(meta->smallest), &(meta->largest));
      if (refined_oldest_ancester_time !=
          std::numeric_limits<uint64_t>::max()) {
        meta->oldest_ancester_time = refined_oldest_ancester_time;
  ...
  if (current_entries == 0 && tp->num_range_deletions == 0) {
    std::string fname =
        TableFileName(compact->immutable_options()->cf_paths,
                      meta->fd.GetNumber(), meta->fd.GetPathId());
    env_->DeleteFile(fname);
  }

  uint64_t oldest_blob_file_number = kInvalidBlobFileNumber;
  Status status_for_listener;
  std::string fname = GetTableFileName(meta->fd.GetNumber());
  FileDescriptor output_fd = meta->fd;
  oldest_blob_file_number = meta->oldest_blob_file_number;
...
Status CompactionJob::GPUCompaction(const Compaction* compact) {
  is_gpu_compact = true;
  log_buffer_->FlushBufferToLog();
  LogCompaction();
  ...
  size_t num_input_level = compact->num_input_levels();

  size_t num_inputs = 0;  // 初始化输入文件数量
  ...
  for (size_t i = 0; i < info_size; ++i) {
    MyOpenCompactionOutputFile(compact, file_writers[i], metas[i], tbs[i]);
  }

  // GPU并行编码多SSTable
  EncodeSSTables(result_d, input_files_d, infos, metas, file_writers, tbs, tps,
                 num_kv_data_block, stream);
  ...
  for (size_t i = 0; i < info_size; ++i) {
    ...
    metas[i].smallest.DecodeFrom(
        Slice(smallest_key.get() + i * (keySize_ + 8), keySize_ + 8));

    metas[i].fd.largest_seqno = largest_seqno[i];
    metas[i].fd.smallest_seqno = smallest_seqno[i];

    MyFinishCompactionOutputFile(compact, file_writers[i], &metas[i], &infos[i],
                                 &tps[i]);
  }
  ...
  for (size_t i = 0; i < tps.size(); ++i) {
    auto fn = TableFileName(compact->immutable_options()->cf_paths,
                            metas[i].fd.GetNumber(), metas[i].fd.GetPathId());
    std::shared_ptr<const TableProperties> table_props;
    table_props = std::make_shared<const TableProperties>(tps[i]);
    tp[fn] = table_props;
  }
  compact_->compaction->SetOutputTableProperties(std::move(tp));
