...
void BlockHandle::MyEncodeTo(std::string* dst) const {
  // Sanity check that all fields have been set
  assert(offset_ != ~uint64_t{0});
  assert(size_ != ~uint64_t{0});
  PutFixed64Varint64(dst, offset_, size_);
}
...
Status BlockHandle::MyDecodeFrom(Slice* input) {
  if (GetFixed64(input, &offset_) && GetVarint64(input, &size_)) {
    return Status::OK();
  } else {
    // reset in case failure after partially decoding
    offset_ = 0;
...
//    Status s = handle.DecodeFrom(input);
    Status s = handle.MyDecodeFrom(input);
    if (!s.ok()) {
      return s;
    }
  }
